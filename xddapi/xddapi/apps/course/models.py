...
from xddapi.utils.models import BaseModel
...
class Course(BaseModel):
    ...
    def lessons_list(self):
        """展示课程列表页中推荐的4个课时信息"""
        data_list = []
        lesson_list = CourseLesson.objects.filter(
            is_show=True,
            is_deleted=False,
            course_id=self.id,  # 获取与当前课程相关的课时信息
            is_show_list=True  # 特定过滤条件，展示在课程列表中
        ).order_by("orders")[:4]  # 只取前4个并按排序字段排序

        for lesson in lesson_list:
            data_list.append({
                "id": lesson.id,
                "name": lesson.name,
                "lesson": lesson.lesson,  # 确保字段名正确
                "free_trial": lesson.free_trail  # 注意字段名
            })
    ...
    def active_list(self):
        """获取当前课程参与的活动"""
        return self.activeprices.filter(is_show=True, is_deleted=False).order_by("-orders", "-id")
...
class CourseDiscountType(BaseModel):
    ...
    class Meta:
        db_table = "xdd_course_discount_type"
        verbose_name = "课程优惠类型"
...
class CoursePriceDiscount(BaseModel):
    ...
    class Meta:
        db_table = "xdd_course_price_dicount"
